...
$terms = "stat_ignored <> 1 AND ";
/* Os chamados com status "ignored" deverão aparecer nessa seção do usuário que os abriu */
$terms = "";
...
$sql = $QRY["tickets_in_queues_count"] . " AND 
            ...
            (s.stat_painel in(3) OR s.stat_ignored = 1 ) 
            ";
...
foreach ($sqlResult->fetchAll() as $row){
    ...
    $nestedData[] = $clientName . $row['contato'];
