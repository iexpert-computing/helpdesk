...
$user_max_cost = $userInfo['max_cost_authorizing'];
...
$sql = $QRY["tickets_in_queues_count"] . " AND stat_ignored <> 1 AND o.authorization_status = 1 AND o.numero IN ({$searchIn})";
...
$sql = $QRY["tickets_in_queues"] . " AND stat_ignored <> 1 AND o.authorization_status = 1 AND o.numero IN ({$searchIn}) ";
...
foreach ($sqlResult->fetchAll() as $row){
    ...
    $ticket_cost = priceDB($cost_field_info['field_value']);


    
    $nestedData[] = ""; /* reservado para o botao de expandir e esconder quando responsivo */
    $nestedData[] = $lastEntryNotification . "{$imgSub}&nbsp;<b>" . $row['numero'] . "</b><br/>" . $row['area'];
    ...
    // $nestedData[] = $colTVNew;
    $nestedData[] = priceScreen($ticket_cost);
